...
class Helpers
{
    ...
    public static function isUsingDashboard()
    {
        return is_admin()
            || is_preview()
            || is_customize_preview()
                ...
                || (strpos($_SERVER['REQUEST_URI'], 'fl_builder') !== false);
    }
    ...
    public static function tzOffsetToName($tz)
    {
        $tz_name = $tz;
        if (strpos($tz, 'UTC') !== false && $tz !== 'UTC') {
            $tz_name = str_replace(array('.25', '.5', '.75', 'UTC'), array(':15', ':30', ':45', ''),
                $tz);
        }
    ...
    public static function wpWeekDayToRRuleDay($days = [])
    {
        $rruleDays = ['SA', 'MO', 'TU', 'WE', 'TH', 'FR', 'SU'];

        return array_map(function ($i) use ($rruleDays) {
            return $rruleDays[$i];
        }, $days);
