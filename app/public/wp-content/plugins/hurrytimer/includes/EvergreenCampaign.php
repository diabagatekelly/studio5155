...
class EvergreenCampaign extends Campaign
{
    ...
    {
        if ($scope === 'admin') {
            $this->IPDetection->forget($this->getId(), Helpers::ip_address());
        } else {
    ...
    {
        $_cookieKey = CookieDetection::cookieName($this->getId());
    ...
    {
        setcookie(CookieDetection:: cookieName($this->getId()), $timestamp,
    ...
    public function reseting()
    {
        ...
        delete_post_meta($this->getId(), self::RESET_FLAG);
